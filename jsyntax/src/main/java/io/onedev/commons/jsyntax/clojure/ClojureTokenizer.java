...
public class ClojureTokenizer extends AbstractTokenizer<ClojureTokenizer.State> {

    static final String BUILTIN = "builtin", COMMENT = "comment", STRING = "string", CHARACTER = "string-2",
            ATOM = "atom", NUMBER = "number", BRACKET = "bracket", KEYWORD = "keyword", VAR = "variable";
    static final int INDENT_WORD_SKIP = 2;
    static final int NORMAL_INDENT_UNIT = 2;
    
    static final Set<String> atoms = wordsOf("true false nil");
    static final Set<String> keywords = wordsOf("defn defn- def def- defonce defmulti defmethod defmacro defstruct deftype defprotocol defrecord defproject deftest " +
    ...
    static final Set<String> builtins = wordsOf("* *' *1 *2 *3 *agent* *allow-unresolved-vars* *assert* *clojure-version* *command-line-args* *compile-files* " +
    ...
    static final Set<String> indentKeys = wordsOf("ns fn def defn defmethod bound-fn if if-not case condp when while when-not when-first do future comment doto " +
    ...
    static class Tests {
    	...
    	Pattern digit_or_colon = Pattern.compile("[\\d:]");
    	Pattern hex = Pattern.compile("[0-9a-f]", Pattern.CASE_INSENSITIVE);
    	Pattern sign = Pattern.compile("[+-]");
    	Pattern exponent = Pattern.compile("e", Pattern.CASE_INSENSITIVE);
    	Pattern keyword_char = Pattern.compile("[^\\s\\(\\[\\;\\)\\]]");
    	Pattern symbol = Pattern.compile("[\\w*+!\\-\\._?:<>\\/\\xa1-\\uffff]");
    	Pattern block_indent = Pattern.compile("^(?:def|with)[^\\/]+$|\\/(?:def|with)");
    }
	...
	static final Pattern x = Pattern.compile("x", Pattern.CASE_INSENSITIVE);
	
	boolean isNumber(String ch, StringStream stream) {
	    if (ch.equals("0") && !stream.eat(x).isEmpty()) {
	...
	static {
	    ...
	    pattern[2] = Pattern.compile("[0-9a-z]{4}", Pattern.CASE_INSENSITIVE);
	}
	
	void eatCharacter(StringStream stream) {
	    String first = stream.next();
	    if (!first.isEmpty() && pattern[0].matcher(first).matches() && !stream.match(pattern[1], true).isEmpty()) {
	        return;
	    }
	    if (first.equals("u")) {
	        stream.match(pattern[2], true);
	    }
	}

	@Override
	public boolean accept(String fileName) {
		return acceptExtensions(fileName, "clj", "cljc", "cljx", "cljs", "edn");
	}

	@Override
	public boolean acceptMime(String mime) {
		return mime != null 
				&& (mime.equals("text/x-clojure")
						|| mime.equals("text/x-clojurescript") 
						|| mime.equals("application/edn"));
	}

	@Override
	public boolean acceptMode(String mode) {
		return mode != null && mode.equals("clojure");
	}
	...
	public String token(StringStream stream, State state) {
	    if (!state.mode.equals("string") && stream.eatSpace()) {
	        return "";
	    }
	    String returnType = "";
	    switch (state.mode) {
	    ...
	    default :
	        ...
	        if (ch.equals("\"")) {
	            ...
	            if (ch.equals("(")) {
	            	while (!(letter = stream.eat(tests.keyword_char)).isEmpty()) {
	            		keyWord += letter;
	            	}
	            }
	            if (keyWord.length() > 0 && (indentKeys.contains(keyWord) || tests.block_indent.matcher(keyWord).matches())) {
	            }
	            else {
	                stream.eatSpace();
	            }
	            ...
	            stream.eatWhile(tests.symbol);
	            if (keywords.contains(stream.current())) {
	                returnType = KEYWORD;
	            }
	            else if (builtins.contains(stream.current())) {
	                returnType = BUILTIN;
	            }
	            else if (atoms.contains(stream.current())) {
	                returnType = ATOM;
	            }
	            else {
	                returnType = VAR;
