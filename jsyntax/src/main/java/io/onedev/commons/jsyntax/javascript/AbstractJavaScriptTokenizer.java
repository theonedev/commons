...
public abstract class AbstractJavaScriptTokenizer extends AbstractTokenizer<AbstractJavaScriptTokenizer.State> {
	...
	private static final Pattern EXPRESSION_ALLOWED_PATTERN1 = Pattern.compile("^(?:operator|sof|keyword c|case|new|[\\[{}\\(,;:]|=>)$");
	
	private static final Pattern EXPRESSION_ALLOWED_PATTERN2 = Pattern.compile("\\{\\s*$");
	
	static boolean expressionAllowed(StringStream stream, State state, int backup) {
		return EXPRESSION_ALLOWED_PATTERN1.matcher(state.lastType).find() ||
		    (state.lastType.equals("quasi") && EXPRESSION_ALLOWED_PATTERN2.matcher(TokenizerUtils.slice(stream.string(), 0, stream.pos() - backup)).find());
	}

	public static boolean expressionAllowed(StringStream stream, State state) {
		return expressionAllowed(stream, state, 0);
	}
	...
	public AbstractJavaScriptTokenizer(boolean typescript) {
		isTS = typescript;
		...
		keywords.put("await", C);
		keywords.put("async", kw("async"));
		
		if (isTS) {
			Keyword type = new Keyword("variable", "variable-3");
			keywords.put("implements", C);
	        keywords.put("namespace", C);
	        keywords.put("module", kw("module"));
	        keywords.put("enum", kw("module"));
	        keywords.put("type", kw("type"));

	        // scope modifiers
	        keywords.put("public", kw("modifier"));
	        keywords.put("private", kw("modifier"));
	        keywords.put("protected", kw("modifier"));
	        keywords.put("abstract", kw("modifier"));

	        // operators
	        keywords.put("as", operator);
	...
	static Pattern TOKEN_BASE_PATTERN31 = Pattern.compile("o", Pattern.CASE_INSENSITIVE);
	
	static Pattern TOKEN_BASE_PATTERN32 = Pattern.compile("b", Pattern.CASE_INSENSITIVE);
	
	static Pattern TOKEN_BASE_PATTERN33 = Pattern.compile("[0-7]", Pattern.CASE_INSENSITIVE);
	
	static Pattern TOKEN_BASE_PATTERN34 = Pattern.compile("[01]", Pattern.CASE_INSENSITIVE);
	...
	class TokenBase implements Processor {
		...
		public String process(StringStream stream, State state) {
		    ...
		    if (ch.equals("\"") || ch.equals("'")) {
		    	...
		    	return ret("number", "number", "", state);
		    } else if (ch.equals("0") && stream.eat(TOKEN_BASE_PATTERN31).length() != 0) {
		        stream.eatWhile(TOKEN_BASE_PATTERN33);
		        return ret("number", "number", "", state);
		    } else if (ch.equals("0") && stream.eat(TOKEN_BASE_PATTERN32).length() != 0) {
		        stream.eatWhile(TOKEN_BASE_PATTERN34);
		        return ret("number", "number", "", state);
	...
	static Pattern FIND_FAT_ARROW_PATTERN2 = Pattern.compile(":\\s*(?:\\w+(?:<[^>]*>|\\[\\])?|\\{[^}]*\\})\\s*$");
	...
	void findFatArrow(StringStream stream, State state) {
	    ...
	    if (isTS) { // Try to skip TypeScript return type declarations after the arguments
	    	Matcher matcher = FIND_FAT_ARROW_PATTERN2.matcher(TokenizerUtils.slice(stream.string(), stream.start(), arrow));
	    	if (matcher.find()) arrow = matcher.start();
	    }
	    ...
	    for (pos = arrow - 1; pos >= 0; --pos) {
	    	...
	    	if (bracket >= 0 && bracket < 3) {
	            ...
	            if (--depth == 0) { if (ch.equals("(")) sawSomething = true; break; }
	...
	class ParenExpr extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (!type.equals("(")) return pass(state);
		    return cont(state, new PushLex(")", ""), new Expression(), new Expect(")"), new PopLex());
		}
	...
	class Statement extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type, String value) {
		    ...
		    if (type.equals("keyword a")) 
		    	return cont(state, new PushLex("form", ""), new ParenExpr(), new Statement(), new PopLex());
		    ...
		    if (type.equals("if")) {
		    	...
		    	return cont(state, new PushLex("form", ""), new ParenExpr(), new Statement(), new PopLex(), new Maybeelse());
		    }
		    ...
		    if (type.equals("switch")) 
		    	return cont(state, new PushLex("form", ""), new ParenExpr(), new PushLex("}", "switch"), new Expect("{"), new Block(), new PopLex(), new PopLex());
		    if (type.equals("case")) 
		    ...
		    if (type.equals("class")) return cont(state, new PushLex("form", ""), new ClassName(), new PopLex());
		    if (type.equals("export")) return cont(state, new PushLex("stat", ""), new AfterExport(), new PopLex());
		    if (type.equals("import")) return cont(state, new PushLex("stat", ""), new AfterImport(), new PopLex());
		    if (type.equals("module")) return cont(state, new PushLex("form", ""), new PatternCombinator(), new PushLex("}", ""), new Expect("{"), new Block(), new PopLex(), new PopLex());
		    if (type.equals("type")) return cont(state, new Typeexpr(), new Expect("operator"), new Typeexpr(), new Expect(";"));
		    if (type == "async") return cont(state, new Statement());
	...
	boolean expressionInner(StringStream stream, State state, String type, boolean noComma) {
		if (state.fatArrowAt == stream.start()) {
	      ...
	      if (type.equals("class")) return cont(state, new PushLex("form", ""), new ClassExpression(), new PopLex());
	      if (type.equals("keyword c") || type.equals("async")) 
	    	  return cont(state, noComma ? new MaybeexpressionNoComma() : new Maybeexpression());
	      if (type.equals("(")) 
	    	  return cont(state, new PushLex(")", ""), new Maybeexpression(), new Expect(")"), new PopLex(), maybeop);
	      ...
	      if (type.equals("new")) return cont(state, maybeTarget(noComma));
	      return cont(state);
	}
	...
	Combinator maybeTarget(boolean noComma) {
		return new Combinator() {

			@Override
			boolean combine(StringStream stream, State state, String type, String value) {
			      if (type.equals(".")) return cont(state, noComma? new TargetNoComma(): new Target());
			      else return pass(state, noComma ? new ExpressionNoComma() : new Expression());
			}
			
		};
	}
	
	class TargetNoComma extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
			if (value.equals("target")) {state.marked = "keyword"; return cont(state, new MaybeoperatorNoComma());}
			return false;
		}
		
	}
	
	class Target extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (value.equals("target")) { state.marked = "keyword"; return cont(state, new MaybeoperatorComma());}
		    return false;
		}
	...
	class Objprop extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type, String value) {
			if (type.equals("async")) {
				state.marked = "property";
				return cont(state, new Objprop());
			} else if (type.equals("variable") || state.style.equals("keyword")) {
		    	state.marked = "property";
	...
	class Commasep extends Combinator {
		...
		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		      if (type.equals(end) || value.equals(end)) 
		    	  return cont(state);
				...
				boolean combine(StringStream stream, State state, String type, String value) {
				      if (type.equals(",")) {
				    	  ...
				    	  if (lex.info.equals("call")) 
				    		  lex.pos = lex.pos + 1;
				          return cont(state, new Combinator() {

							@Override
							boolean combine(StringStream stream, State state, String type, String value) {
						          if (type.equals(end) || value.equals(end)) return pass(state);
						          return pass(state, what);
							}
	...
	class Maybetype extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (isTS) {
		        if (type.equals(":")) return cont(state, new Typeexpr());
		        if (value.equals("?")) return cont(state, new Maybetype());
		    }
			return false;
		}
	...
	class Typeexpr extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (type.equals("variable")) {state.marked = "variable-3"; return cont(state, new AfterType());}
		    if (type.equals("{")) return cont(state, new Commasep(new Typeprop(), "}"));
		    if (type.equals("(")) return cont(state, new Commasep(new Typearg(), ")"), new MaybeReturnType());
		    return false;
		}
		
	}
	
	class MaybeReturnType extends Combinator {
	...
	class Typeprop extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (type.equals("variable") || state.style.equals("keyword")) {
		    	state.marked = "property";
		    	return cont(state, new Typeprop());
		    } else if (type.equals(":")) {
		    	return cont(state, new Typeexpr());
		    }
		    return false;
		}
	...
	class Typearg extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (type.equals("variable")) return cont(state, new Typearg());
		    else if (type.equals(":")) return cont(state, new Typeexpr());
		    return false;
		}
		
	}
	
	class AfterType extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (value.equals("<")) return cont(state, new Commasep(new Typeexpr(), ">"), new AfterType());
		    if (type.equals("[")) return cont(state, new Expect("]"), new AfterType());
		    return false;
		}
	...
	class PatternCombinator extends Combinator {
		...
		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (type.equals("modifier")) return cont(state, new PatternCombinator());
		    if (type.equals("variable")) { 
		    	register(state, value); 
		    	return cont(state); 
		    }
		    if (type.equals("spread")) return cont(state, new PatternCombinator());
	...
	class Functiondef extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type,
		    ...
		    if (type.equals("(")) return cont(state, new PushContext(), new PushLex(")", ""), new Commasep(new Funarg(), ")"), new PopLex(), new Maybetype(), new Statement(), new PopContext());
		    return false;
		}
	...
	class ClassExpression extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (type.equals("variable")) return new ClassName().combine(stream, state, type, value);
		    return new ClassNameAfter().combine(stream, state, type, value);
		}
	...
	class ClassNameAfter extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type,
				String value) {
		    if (value.equals("extends") || value.equals("implements")) return cont(state, isTS ? new Typeexpr(): new Expression(), new ClassNameAfter());
		    if (type.equals("{")) 
	...
	private static final Pattern CLASSBODY_PATTERN = Pattern.compile("^\\s+[\\w$\\xa1-\\uffff]");
	...
	class ClassBody extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type,
				String value) {
		    if (type.equals("variable") || state.style.equals("keyword")) {
		        if ((value.equals("static") || value.equals("get") || value.equals("set") ||
		                (isTS && (value.equals("public") || value.equals("private") || value.equals("protected") || value.equals("readonly") || value.equals("abstract")))) &&
		               !stream.match(CLASSBODY_PATTERN, false).isEmpty()) {
		        	state.marked = "keyword";
		        	return cont(state, new ClassBody());
		        }
		        state.marked = "property";
		        return cont(state, isTS ? new Classfield(): new Functiondef(), new ClassBody());
		    }
		    if (value.equals("*")) {
		    	state.marked = "keyword";
		    	return cont(state, new ClassBody());
	...
	class Classfield extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (value.equals("?")) return cont(state, new Classfield());
		    if (type.equals(":")) return cont(state, new Typeexpr(), new MaybeAssign());
		    return pass(state, new Functiondef());
		}
	...
	class ImportSpec extends Combinator {
		...
		boolean combine(StringStream stream, State state, String type,
		    ...
		    if (value.equals("*")) state.marked = "keyword";
		    return cont(state, new MaybeAs());
		}
		
	}
	
	class MaybeAs extends Combinator {

		@Override
		boolean combine(StringStream stream, State state, String type, String value) {
		    if (value.equals("as")) {state.marked = "keyword"; return cont(state, new ImportSpec());}
		    return false;
		}
	...
	public void skipExpression(State state) {
		if (state.cc.isEmpty())
			return;
		Combinator top = state.cc.peek();
		if (top instanceof Expression || top instanceof ExpressionNoComma)
			state.cc.pop();
	}
