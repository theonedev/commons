...
public class InputSuggestionTest {
	...
	@Test
	public void test() {
		InputSuggestion suggestion = new InputSuggestion("hello world", new Range(0, 5));
		suggestion = suggestion.escape("");
		assertEquals("hello world", suggestion.getContent());
		assertEquals("0-5", suggestion.getMatch().toString());
		
		suggestion = new InputSuggestion("'hello'world", new Range(0, 12));
		suggestion = suggestion.escape("'");
		assertEquals("\\'hello\\'world", suggestion.getContent());
		assertEquals("0-14", suggestion.getMatch().toString());
		
		suggestion = new InputSuggestion("'hello'wor'ld", new Range(3, 12));
		suggestion = suggestion.escape("'");
